...
public class CommentController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<CommentResponse>> getCommentById(@PathVariable Long id, @AuthenticationPrincipal CustomUserDetails customUserDetails) {
        System.out.println("comment id" + id);
        ApiResponse<CommentResponse> response = commentService.getCommentById(id, customUserDetails.getId());
        return new ResponseEntity<>(response, response.getStatus());
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<ApiResponse<CommentResponse>> deleteCommentById(@PathVariable Long id, @AuthenticationPrincipal CustomUserDetails customUserDetails) {
